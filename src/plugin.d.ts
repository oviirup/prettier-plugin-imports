...
export interface PluginConfig {
   ...
   * you can use `<THIRD_PARTY_MODULES>` to assign third party imports to the
   ...
   * can use the special `<TYPES>` string. This example will place third party
   * types at the top, followed by local types, then third party value imports,
   ...
   * babel parser, so it can understand the syntaxes used in the file being
   * formatted. The plugin uses prettier itself to figure out the parser it
   * needs to use but if that fails, you can use this field to enforce the usage
