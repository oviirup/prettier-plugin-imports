 ...
 *
 * Nodes are only sorted within the same chunk, but different chunks keep their
 * relative order. This is used, e.g., to keep the order of side-effect
 * imports.
 *
 ...
 * - Otherwise, if the node is preceded by a comment exactly equal (up to leading
 *   and trailing spaces) the string `prettier-ignore`, classify the node as
 *   `unsortable`.
